<template>
  <div class="detail">
    <!-- 商品分类导航 -->
    <type-nav :isshow="true" :startshow="false"></type-nav>
    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <zoom-my :imglist="skuInfo.skuImageList"></zoom-my>
          <!-- 小图列表 -->
          <image-list :imglist="skuInfo.skuImageList"></image-list>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  ...
                  :key="item1.id"
                  @click="changeAttr(item.spuSaleAttrValueList, item1)"
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="addcartsuccess()">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
import ImageList from "./childrencomps/ImageList/ImageList.vue";
import ZoomMy from "./childrencomps/Zoom/ZoomMy.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const skuInfo = computed(() => store.state.detali.detalidata.skuInfo || {});
    const spuSaleAttrList: any = computed(
      () => store.state.detali.detalidata.spuSaleAttrList
    );
    store.dispatch("getdetalidata", route.query.skuId);
    const changeAttr = (attr: any, attrvalue: any) => {
    ...
    const addcartsuccess = async () => {
      const result: any = await reqaddorupdatacart(route.query.skuId, count.value);
      if (result.code === 200) {
        ElMessage({
          message: '商品以成功添加到购物车',
          type: 'success',
      ...
      let cart = JSON.stringify(skuInfo.value);
